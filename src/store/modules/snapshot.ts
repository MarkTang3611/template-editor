    ...
    async unDo() {
      ...
      switch (type) {
        ...
        case SnapshotType.MODIFY: {
          ...
          const _obj: any = await FabricInstance(obj);
          if (_obj.type === ElementNames.ACTIVE && Array.isArray(_obj._objects) && _obj._objects.length > 0) {
            _obj._objects.forEach((element) => {
              const findElementIndex = canvas._objects.findIndex((item) => item.id === element.id);
              canvas.remove(canvas._objects[findElementIndex]);
              canvas.insertAt(findElementIndex, element);
            });
          } else {
            canvas.remove(canvas._objects[findIndex]);
