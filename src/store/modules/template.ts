...
export const useTemplatesStore = defineStore('Templates', {
  ...
  actions: {
    ...
    modifedElement(target: FabricObject, options: Record<string, any>) {
      ...
      const transform = target.toObject(propertiesToInclude);
      console.log('加入快照的旧数据', transform);
      target.set({ ...options });
      const data: Snapshot = {
        type: SnapshotType.MODIFY,
        index,
        transform: transform, // 修改之前的旧数据
        tid: this.templateId,
        target: target.toObject(propertiesToInclude) // 修改之后的新数据
      };
      const proxyEl: any = this.currentTemplate.objects.find((i) => i.id === target.id);
      console.log('加入快照的新数据：', data);
    ...
    deleteTemplate(templateId: string | string[]) {
      ...
      const deleteTemplatesIndex: number[] = [];
    ...
    clearTemplate() {
      const objects = this.templates[this.templateIndex].objects.filter(
        (item) => item.id === WorkSpaceDrawType
      );
    ...
    updateElement(data: UpdateElementData) {
      ...
      const elements = template.objects.map((el) =>
        elementIds.includes(el.id) ? { ...el, ...props } : el
      );
