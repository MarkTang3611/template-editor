...
import { WorkSpaceDrawType, CanvasBackground, WorkSpaceDrawData } from '@/configs/canvas';
...
export default () => {
  ...
  const getEmptyTemplate = (w = 800, h = 400, options: any = {}): Template => {
    WorkSpaceDrawData.width = w;
    WorkSpaceDrawData.height = h;
    const emptyTemplate: Template = {
      ...
      height: h,
      clip: 2,
      objects: [WorkSpaceDrawData as any],
      workSpace: {
        fillType: 0,
        left: 0,
        top: 0,
    ...
    // console.log(emptyTemplate.objects[0]);
  ...
  // 创建一页空白页并添加到下一页
  const createTemplate = async (w = 800, h = 400, options = {}) => {
    await templatesStore.addTemplate(getEmptyTemplate(w, h, options));
    // await templatesStore.renderTemplate();
  };
