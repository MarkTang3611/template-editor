...
import { ActiveSelection, FabricObject, Group, Path } from 'fabric';
...
export default () => {
  ...
  const sortElement = async (eventData: {
    moved: { newIndex: number; oldIndex: number; element: FabricObject };
  }) => {
    ...
    if (!element) return;
    if (element.group) {
      const elementGroup = queryOption((element.group as GroupElement).id) as
        | GroupElement
        | undefined;
  ...
  const deleteTextbox = (element: TextboxElement): boolean => {
    ...
    if (element.isEditing) {
      ...
      if (selectedText) {
        ...
        textboxElement.removeChars(
          textboxElement.selectionStart,
          textboxElement.selectionStart + 1
        );
      }
      canvas.renderAll();
      return true;
  ...
  const deleteElement = (eid: string) => {
    ...
    if (element.group) {
      if ((element.group as GroupElement)._objects.length === 1) {
        ...
        if (element.type === ElementNames.TEXTBOX && deleteTextbox(element as TextboxElement))
          return;
  ...
  const moveElement = (command: string, step = 2) => {
    ...
    templatesStore.updateElement({
      id: activeObject.id,
      props: activeObject.toObject(propertiesToInclude as any[])
    });
  };
  ...
  const queryElement = (eid: string): FabricObject | undefined => {
    ...
    const elements = canvas
      .getObjects()
      .filter((item) => !WorkSpaceCommonType.includes((item as FabricObject).id));
  ...
  const selectAllElement = () => {
    ...
    const allObjects = canvas
      .getObjects()
      .filter((obj) => obj.selectable !== false && !obj.lockMovementX && !obj.lockMovementY);
    if (allObjects.length > 0) {
      const newSelection = new ActiveSelection(allObjects, { canvas: canvas });
      canvas.setActiveObject(newSelection);
    } else {
      canvas.discardActiveObject(); // 如果没有可选的，则清空选择
    }
    canvas.requestRenderAll();
  };
